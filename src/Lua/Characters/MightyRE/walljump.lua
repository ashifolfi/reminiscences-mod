...
addHook("ThinkFrame", function()
	for player in players.iterate do
		if (player.mo and player.mo.skin == "mightyre") then
			-- if not (player.atwall)
				player.atwall = 0
			-- end
			if not (player.cling) then
				player.cling = false
			end
			if not (player.clingprev) then
				player.clingprev = false
			end
			if not (player.bounce) then
				player.bounce = false
			end
			if player.bounce
			...
			if (player.mo) then -- Fixes spectating, and possibly other things
				...
				if (fheight > player.mo.z) or ((cheight <= player.mo.height+player.mo.z) and not (fheight == cheight)/* and (wall.ceilingpic == "F_SKY1")*/) then -- This last bit seems to be broken...
					player.atwall = 1
				end
				-- FOFs can be walls too, so lets check for those
				for wall in wall.ffloors() do
				...
				and not (player.charability2 == CA2_MULTIABILITY) then
					...
					and not (player.exiting) then
						P_SetObjectMomZ(player.mo, -3*FRACUNIT)
						player.mo.momx = $1 / 2
						player.mo.momy = $1 / 2
						player.mo.state = S_PLAY_CLING -- F
						player.cling = true
						if (player.clingprev == false) then
							S_StartSound(player.mo,sfx_s3k4a)
						end
						if not (S_SoundPlaying(player.mo,sfx_s3k7e)) then
