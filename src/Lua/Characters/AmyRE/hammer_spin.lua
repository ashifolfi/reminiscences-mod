...
addHook("ThinkFrame", function(player)
	for player in players.iterate do
		...
		if player.mo.skin == "amyre" then -- charname here
			...
			if player.hammerspincooldown == nil then
				player.hammerspincooldown = 0
			end
			if player.mo.state == S_AMYRE_HAMMERSPIN
			and player.hammerspincooldown == 0 then
				player.hammerspincooldown = 64
			end
			if player.hammerspincooldown > 0 then
				player.hammerspincooldown = $ - 1
			end
			-- Ghost trail thing
			if player.mo.state == S_AMYRE_HAMMERSPIN then
...
local function HammerSpinKill(touchedmobj, mobj)
	...
	if mobj.z + mobj.height < touchedmobj.z then return end
	if mobj.z > touchedmobj.z then return end
	if (player.mo.skin == "amyre")
	and (touchedmobj.flags & (MF_ENEMY|MF_BOSS))
	and (player.mo.state == S_AMYRE_HAMMERSPIN) then
