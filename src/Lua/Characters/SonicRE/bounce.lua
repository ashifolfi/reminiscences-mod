...
addHook("MobjThinker", function(sonicre)
	...
	and not (sonicre.player.powers[pw_super]) then -- super sonicre has float instead
		if sonicre.player.bounceattack == nil then -- if the variable doesnt exist
			sonicre.player.bounceattack = 0 -- create variable
		end
		if sonicre.player.bouncenumber == nil then -- if the variable doesnt exist
		...
		and sonicre.player.bounceattack == 0 then -- the player isnt bouncing
			sonicre.player.bouncenumber = 0 -- set it to zero!
		end
		if sonicre.player.usedown == nil -- if the variable doesnt exist
		or not (sonicre.player.cmd.buttons & BT_SPIN) then -- not pressing the spin button?
			sonicre.player.usedown = 0 -- create variable
			sonicre.player.justchangedbouncenumber = 0 -- create variable
		end
		if P_IsObjectOnGround(sonicre) -- Is the player on the ground?
		and (sonicre.player.cmd.buttons & BT_SPIN) then -- Is the player holding the spin button?
		...
		and not (P_PlayerInPain(sonicre.player)) then -- not hurt are you?
		...
		and not (P_PlayerInPain(sonicre.player)) then -- not hurt are you?
			...
			and sonicre.player.justchangedbouncenumber == 0 then -- if he hasnt left the ground yet
				...
				if not (sonicre.player.panim == PA_ROLL) then -- checking if the players animation is it's spinning anim.
