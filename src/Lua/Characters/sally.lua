...
addHook("AbilitySpecial", function(player)
	...
	and not(player.mo.state == S_PLAY_ROLL) then -- ...and you're not in roll state...
...
addHook("ThinkFrame", function(player)
	for player in players.iterate do
		...
		and (P_IsObjectOnGround(player.mo)) then -- ...and you're on the ground...
		...
		if player.sallyslide == nil then -- If the sallyslide value doesn't exist...
			player.sallyslide = false -- ...create it on false!
		end

		if player.sallyslide == true -- If the sallyslide value is true...
		and not(player.pflags & PF_SPINNING) then -- ...but you're not actually sliding...
			player.sallyslide = false -- ...change the value to false!
		end

		-- sliding dust thingy
		if player.mo.state == S_SALLYRE_SLIDE then --are you in the slide state?
...
addHook("JumpSpecial", function(player)
	...
	and (player.mo.state == S_PLAY_SPINDASH or player.sallyslide == true) then -- ...and you're crouching or sliding...
